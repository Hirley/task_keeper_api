...
RSpec.describe 'Webhooks (tela web)', type: :request do
  ...
  describe 'GET /webhooks' do
    it 'lista os webhooks cadastrados pro líder' do
      create(:webhook_subscription, url: 'http://8.8.8.8/hook', user: lider)
      sign_in lider
  ...
  describe 'POST /webhooks' do
    it 'cadastra um webhook com os eventos escolhidos' do
      ...
      expect(webhook.user).to eq(lider)
    end
  ...
  describe 'PATCH /webhooks/:id' do
    it 'atualiza os eventos e permite pausar (active: false)' do
      webhook = create(:webhook_subscription, events: %w[demanda_criada], user: lider)
      sign_in lider
  ...
  describe 'DELETE /webhooks/:id' do
    it 'remove o webhook' do
      webhook = create(:webhook_subscription, user: lider)
      sign_in lider
    ...
    it 'pede confirmação antes de excluir' do
      create(:webhook_subscription, user: lider)
      sign_in lider

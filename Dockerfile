...
# num passo posterior. Copiando tudo de uma vez só, isso não acontece —
# o custo é que qualquer mudança no código também invalida o cache do
# bundle install, não só mudanças no Gemfile/Gemfile.lock.
COPY . .
